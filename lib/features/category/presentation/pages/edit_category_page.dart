...
class _EditCategoryPageState extends ConsumerState<EditCategoryPage> {
  ...
  void _initializeForm(CategoryEntity category) {
    if (_isInitialized) return;
    _isInitialized = true;
    _initialCategory = category;
    _namaController.text = category.nama;
    _selectedIcon = category.ikon;
    _selectedColor = category.warna;
  }
  ...
  Future<void> _submit() async {
    ...
    if (_initialCategory == null) return;

    final updatedCategory = _initialCategory!.copyWith(
      nama: nama,
      ikon: _selectedIcon!,
    ...
    try {
      ...
      if (mounted) {
        await FeedbackDialog.showSuccess<void>(
          ...
          message: 'Perubahan kategori sudah disimpan.',
        );
      }
      if (mounted) {
        context.pop();
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            const Text('Kategori tidak ditemukan'),
