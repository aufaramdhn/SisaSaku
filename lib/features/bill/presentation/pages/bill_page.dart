...
class _BillPageState extends ConsumerState<BillPage> {
                      ...
                      children: [
                        ...
                        billsAsync.when(
                          data: (bills) {
                            ...
                            final upcoming = bills.where((b) => b.status == BillStatus.upcoming || b.status == BillStatus.pending).toList();
                            final paid = bills.where((b) => b.status == BillStatus.paid).toList();

                            return Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                _buildFilterChips(overdue.length, upcoming.length, paid.length),
                                const SizedBox(height: AppSpacing.xl),
  ...
  Widget _buildBillList(List<BillEntity> overdue, List<BillEntity> upcoming, List<BillEntity> paid) {
    final allWidgets = <Widget>[];

    if (overdue.isNotEmpty && (_filter == 'all' || _filter == 'overdue')) {
      allWidgets.add(_buildSectionHeader(Icons.error, 'Jatuh Tempo Terlewat', AppColors.dangerColor));
      allWidgets.add(const SizedBox(height: AppSpacing.md));
      for (final bill in overdue) {
        allWidgets.add(_BillCard(
          icon: Icons.wifi,
    ...
    if (upcoming.isNotEmpty && (_filter == 'all' || _filter == 'upcoming')) {
      allWidgets.add(_buildSectionHeader(Icons.warning, 'Mendekati', AppColors.warningDark));
      allWidgets.add(const SizedBox(height: AppSpacing.md));
    ...
    if (paid.isNotEmpty && (_filter == 'all' || _filter == 'paid')) {
      allWidgets.add(_buildSectionHeader(Icons.check_circle, 'Sudah Lunas', AppColors.successColor));
      allWidgets.add(const SizedBox(height: AppSpacing.md));
  ...
  Widget _buildFilterChips(int overdueCount, int upcomingCount, int paidCount) {
    final filters = [
      ('Overdue ($overdueCount)', 'overdue', AppColors.dangerLight, AppColors.dangerColor, const Color(0xFFFFDAD6)),
      ('Mendekati ($upcomingCount)', 'upcoming', AppColors.warningLight, AppColors.warningDark, const Color(0xFFFFDDB7)),
      ('Lunas ($paidCount)', 'paid', AppColors.bgPrimary, AppColors.textSecondary, AppColors.borderColor),
    ];
        ...
        children: [
          for (final (label, value, bgColor, textColor, borderColor) in filters)
            Padding(
              ...
              child: Material(
                color: _filter == value || (_filter == 'all' && value != '') ? bgColor : AppColors.bgPrimary,
                borderRadius: BorderRadius.circular(AppRadius.full),
                child: InkWell(
                  onTap: () {
                    setState(() {
